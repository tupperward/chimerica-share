...
abilities_bot:
  - name: "Clutch"
    desc: "⬻ __Requirements__ The giant dragonfly has a Medium or smaller creature [[conditions#grabbed|grabbed]] in its mandibles  __Effect__  The dragonfly tries to transfer the [[conditions#grabbed|grabbed]] creature to be clutched by its legs. The giant dragonfly attempts an [[Athletics]] check against the creature's Reflex DC. On a success, it transfers the creature (which remains [[conditions#grabbed|grabbed]]) to its legs, freeing its mandibles to attack. The dragonfly can have only one creature clutched at a time."
  - name: "Snatch"
